...
public class AltaProyecto implements WindowListener, ActionListener {
	...
	public AltaProyecto() {
		...
		try {
			...
			while (rs.next()) {
				choClienteProyecto.add(rs.getInt("idCliente") + "-" + rs.getString("nombreCliente") + "-"
	...
	public void actionPerformed(ActionEvent evento) {
		if (evento.getSource().equals(btnAceptarAltaProyecto)) {
			...
			try {
				// Crear una sentencia
				statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
				if (((txtNombreProyecto.getText().length()) != 0) && ((txtFechaInicioProyecto.getText().length()) != 0)
						&& ((txtFechaFinProyecto.getText().length()) != 0)) {
					String[] clientes = (choClienteProyecto.getSelectedItem()).split("-");
					// clientes[0] --> idCliente
					// clientes[1] --> nombre
					// clientes[2] --> Cif
					sentencia = "INSERT INTO proyectos VALUES(null, '" + txtNombreProyecto.getText() + "', '"
							+ txtFechaInicioProyecto.getText() + "', '" + txtFechaFinProyecto.getText() + "', "
							+ clientes[0] + ")";
