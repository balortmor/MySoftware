...
public class MySoftware implements WindowListener, ActionListener {
	...
	// Ventana de Modificación de Cliente
	Frame frmModificarCliente = new Frame("Modificación de Cliente");
	Label lblEditarCliente = new Label("Seleccionar el cliente:");
	Choice choClienteEditar = new Choice();
	...
	public void actionPerformed(ActionEvent evento) {
		...
		if (evento.getSource().equals(mniAltaCliente)) {
			...
			try {
				...
				if (((txtNombreCliente.getText().length()) != 0) && ((txtCifCliente.getText().length()) != 0)) {
					sentencia = "INSERT INTO clientes VALUES (null, '" + txtNombreCliente.getText() + "', '"
			...
			try {
				...
				// y ejecutar la sentencia SQL
				rs = statement.executeQuery(sentencia);
				choClienteEditar.removeAll();
				while (rs.next()) {
					choClienteEditar.add(rs.getInt("idCliente") + "-" + rs.getString("nombreCliente") + "-"
							+ rs.getString("cifCliente"));
				}
			} catch (SQLException sqle) {

			}
			frmModificarCliente.add(choClienteEditar);
			...
			new EditarCliente(choClienteEditar.getSelectedItem());
